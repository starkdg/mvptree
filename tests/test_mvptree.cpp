...
void simple_test(){
	...
	for (DataPoint<> *dp : results){
		H64DataPoint<> *pnt = (H64DataPoint<>*)dp;
	...
	const H64DataPoint<> *retnode = (H64DataPoint<>*)tree.Lookup(10);
	assert(retnode != NULL);
...
void test(){
	...
	const DataPoint<> *dp = mvptree.Lookup(100);
	cout << "Lookup 100 => " << dp->GetId() << endl;
	assert(dp != NULL);
	
	const DataPoint<> *dp2 = mvptree.Lookup(155);
