...
const int lpn = 6;  // levelspernode
const int fo = 64; //fanout bf^lpn
const int ns = 32; //numsplits bf^(lpn-1)
...
int generate_data(vector<DataPoint*> &points, int N){
	...
	for (int i=0;i<N;i++){
		...
		DataPoint *pnt = new DataPoint(m_id++, value);
...
int generate_cluster(vector<DataPoint*> &points, uint64_t center, int N, int max_radius){
	...
	DataPoint *mid = new DataPoint(m_id++, center);
	...
	for (int i=0;i < N-1;i++){
		...
		DataPoint *pnt = new DataPoint(m_id++, val);
		points.push_back(pnt);
	}
	return N;
}
...
void do_run(MVPTree<bf,pl,lc,lpn,fo,ns> &mvptree, vector<PerfMetrics> &metrics){
	...
	for (int i=0;i < n_centers;i++){
		auto s = chrono::steady_clock::now();
		vector<DataPoint*> results = mvptree.Query(centers[i], radius);
 		auto e = chrono::steady_clock::now();
		total_query += (e - s);
	}
...
int main(int argc, char **argv){
	...
	cout << "Indexing " << n*n_iters << " datapoints" << endl;
	cout << "Indexing " << n_centers << " clusters of size " << cluster_size << " with radius = " << radius << endl;
	cout << "Avg. across " << n_runs << " runs" << endl;
	...
	cout << endl << "avg: build ops: " << avg_build_ops << " % opers" 
		 << " build time: " << avg_build_time << " secs" << endl
		 << " avg query ops: " << avg_query_ops << " % opers"
		 << " avg query time: " << avg_query_time << " microsecs" << endl;
	cout << "Done." << endl;
	return 0;
}
